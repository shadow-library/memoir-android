...
class EncryptedConfigStore @Inject constructor(@ApplicationContext context: Context) {
    ...
    private val secretKey: SecretKey
        get() {
            ...
            (ks.getEntry(KEYSTORE_ALIAS, null) as? KeyStore.SecretKeyEntry)
    ...
    fun saveFirebaseCredentials(
        ...
        prefs.edit()
            .putString(KEY_PROJECT_ID,     encrypt(projectId))
            .putString(KEY_APP_ID,         encrypt(appId))
            .putString(KEY_API_KEY,        encrypt(apiKey))
            .putString(KEY_STORAGE_BUCKET, encrypt(storageBucket))
            .apply { if (databaseUrl != null) putString(KEY_DATABASE_URL, encrypt(databaseUrl)) else remove(KEY_DATABASE_URL) }
            .apply()
    }
    ...
    fun hasFirebaseCredentials(): Boolean =
        prefs.getString(KEY_PROJECT_ID, null) != null
}
